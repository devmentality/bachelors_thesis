...
Op MakeInsertOpFromValues(
    ...
    Op op{ Uuid{"0-0-A"}, Uuid{"yarn"} };  // Just fake id & ref. Will be generated by oplog?
...
PROC SerializeToRon(const Op& op) {
    RONtStream std{};
    CALL(std.Open("-", Stream::WRITE));  
    CALL(std.FeedOp(op));                
    CALL(std.FeedEOF());    
    CALL(std.Drain()); 
    DONE;
...
PROC HandleInsert(sqlite3 *db, const TableDescription& table) {
    auto values = ReadNewValues(db);

    auto areValidTypes = AreValidTypes(table, values);
    cout << "Types are " << (areValidTypes ? "" : "not") << " valid" << endl;

    CALL(SerializeToRon(MakeInsertOpFromValues(table, values)));
    DONE;
}


PROC HandleDelete(sqlite3* db, const TableDescription& table) {
    auto values = ReadOldValues(db);

    CALL(SerializeToRon(MakeDeleteOpFromValues(table, values)));
    DONE;
}
...
void PreupdateHook(
    ...
    if (operation == SQLITE_INSERT) {
        MUST_OK(HandleInsert(db, tbl), "insert failed");
    } else if (operation == SQLITE_DELETE) {
        MUST_OK(HandleDelete(db, tbl), "delete failed");
    } else {
...
void SetupHooks(sqlite3* db, vector<TableDescription> *tracked_tables) {
    sqlite3_preupdate_hook(db, PreupdateHook, tracked_tables);
    sqlite3_commit_hook(db, CommitHook, NULL);
}