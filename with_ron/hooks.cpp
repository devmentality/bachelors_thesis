...
PROC HandleInsert(sqlite3 *db, const TableDescription& table, Uuid op_uuid) {
    auto values = ReadNewValues(db);

    auto areValidTypes = AreValidTypes(table, values);
    cout << "Types are " << (areValidTypes ? "" : "not") << " valid" << endl;

    CALL(SerializeToRon(MakeInsertOpFromValues(table, values, op_uuid)));
    DONE;
}


PROC HandleDelete(sqlite3* db, const TableDescription& table, Uuid op_uuid) {
    auto values = ReadOldValues(db);

    CALL(SerializeToRon(MakeDeleteOpFromValues(table, values, op_uuid)));
    DONE;
}
...
void PreupdateHook(
    ...
    if (operation == SQLITE_INSERT) {
        MUST_OK(HandleInsert(db, tbl, op_uuid), "insert failed");
    } else if (operation == SQLITE_DELETE) {
        MUST_OK(HandleDelete(db, tbl, op_uuid), "delete failed");
    } else {
